...
@section('content')
    <div class="m-5 rounded-lg lg:flex gap-2 items-center bg-white">
        ...
        <form action="{{ route('publications.store') }}" method="POST" class="mt-2 lg:w-2/3 lg:pr-5 lg:pb-4">
            @csrf
            <fieldset class="p-5 border-4 border-gray-200 rounded-lg md:grid md:grid-cols-3 md:gap-5">
                ...
                <div class="mb-3">
                    ...
                    <select name="category" id="category" class="border p-3 w-full rounded-lg @error('category') border-red-500 @enderror">
                        <option value="{{ old('category') }}" selected>{{ old('category') ? old('category') : 'Choose' }}</option>
                ...
                <div class="col-start-1 col-span-1">
                    ...
                    <select name="condition" id="condition" class="border p-3 w-full rounded-lg @error('condition') border-red-500 @enderror">
                        <option value="{{ old('condition') }}" selected>{{ old('condition') ? old('condition') : 'Choose' }}</option>
