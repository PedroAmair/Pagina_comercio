...
@section('content')
    <div class="m-5 rounded-lg flex items-center justify-center bg-white">
        <form action="{{route('directions.update', [auth()->user()->username, $bdData])}}" method="POST" class="mt-2 xl:w-2/5 xl:pr-5 xl:pb-4">
            @csrf
            @method('PATCH')
            <fieldset class="p-5 border-4 border-gray-200 rounded-lg">
                ...
                <div>
                    <livewire:location-component :bdData="$bdData" />
                </div>
